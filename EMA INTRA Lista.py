...
for ativo in ativos:
    try:
        ...
        # Adiciona o indicador EMA no dataframe
        rates_frame['EMA'] = ta.trend.ema_indicator(rates_frame['close'], window=PERIODO, fillna=False)

        rates_frame['Buy_Signal'] = (rates_frame['close'] > rates_frame['EMA'])
        rates_frame['Sell_Signal'] = (rates_frame['close'] < rates_frame['EMA'])
...
performance_df.to_excel(f"EMA_{PERIODO}_INTRA_{time_frame}.xlsx", index=False)
