...
for index, row in rates_frame.iterrows():
    ...
    elif position == 1 and (row['Sell_Signal'] or (row['close'] - entry_price) <= STOPLOSS):
        position = 0
...
trade_records_df.to_excel(f"operaçoes_intra_{ativo}_{RSI_ENTRADA}_{RSI_SAIDA}_M15_com Stop.xlsx")
