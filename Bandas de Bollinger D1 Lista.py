...
for ativo in ativos:
    try:
        ...
        # Adiciona o indicador no dataframe
        rates_frame['H_BAND'] = ta.volatility.bollinger_hband_indicator(rates_frame['Close'], window=PERIODO_BB, fillna=False, window_dev=2)
        rates_frame['L_BAND'] = ta.volatility.bollinger_lband_indicator(rates_frame['Close'], window=PERIODO_BB, fillna=False, window_dev=2)
        rates_frame['M_BAND'] = ta.volatility.bollinger_mavg(rates_frame['Close'], fillna=False)

        rates_frame['Buy_Signal'] = (rates_frame['H_BAND'] == 1)
        #rates_frame['Buy_Signal'] = (rates_frame['Close'] < rates_frame['M_BAND'])
        rates_frame['Sell_Signal'] = (rates_frame['L_BAND'] == 1)
        #rates_frame['Sell_Signal'] = (rates_frame['Close'] > rates_frame['M_BAND'])
...
performance_df.to_excel(F"BANDA DE BOLLINGER_{PERIODO_BB}_REV_D1.xlsx", index=False)
